...
import 'package:hotel/screens/rooms/room_cleaning_product_view.dart';
...
class _RoomBeforeCleaningViewState extends State<RoomBeforeCleaningView> {
  ...
  void initState() {
    super.initState();
    _roomService = RoomService(env: widget.env);
  }
  ...
  Future<void> _navigateAndRetrieveSelectedProducts(BuildContext context) async {
    List<ProductCleanning>? result = await Navigator.push(
      context, 
      MaterialPageRoute(
        builder: (BuildContext context) => 
          RoomCleaningProductView(
            env: widget.env,
            room: widget.room, 
            cleanningProductsId: cleanningProductsId, 
